...
from skrobot.model.primitives import Axis, Box, PointCloudLink, Sphere
from skrobot.sdf.signed_distance_function import SignedDistanceFunction, UnionSDF
...
class Camera(Axis):
    ...
    def look_at(self, p: np.ndarray) -> None:
    ...
    def generate_point_cloud(
        self, sdf: SignedDistanceFunction, max_dist: float = 5.0
    ) -> np.ndarray:
        co_proj_plane = self.copy_worldcoords()
        co_proj_plane.translate(np.array([max_dist, 0.0, 0.0]))

        half_width = np.tan(self.config.fovx * 0.5) * max_dist
        half_height = np.tan(self.config.fovy * 0.5) * max_dist
        ...
        dists = self.ray_marching(ray_starts, ray_directions_unit, sdf, 1e-3, max_dist=max_dist)
        clouds = ray_starts + dists[:, None] * ray_directions_unit
        # return clouds[dists < max_dist - 1e-3]
        return clouds

    @staticmethod
    def ray_marching(
        pts_starts, direction_arr_unit, f_sdf, threshold, max_iter: int = 30, max_dist: float = 5.0
    ) -> np.ndarray:
        ...
        for _ in range(max_iter):
            ...
            is_close_enough = sd_vals_active < threshold
            active_flags[active_indices[is_close_enough]] = False

            is_far_enough = frying_dists[active_indices] > max_dist
            active_flags[active_indices[is_far_enough]] = False
            frying_dists[active_indices[is_far_enough]] = max_dist
            ray_tips[active_indices[is_far_enough]] = np.inf

            active_ratio = np.sum(active_flags) / n_point
            if active_ratio < 0.005:
                break

        # frying_dists[active_indices] = np.inf
        modified_dists = np.minimum(frying_dists, 5.0)
...
if __name__ == "__main__":
    config = CameraConfig(resolx=320, resoly=240)
    camera = Camera(np.array([0.0, 0.0, 0.0]), config=config)
    ...
    for _ in range(5):
        ...
        pts = camera_tmp.generate_point_cloud(sdf=sdf, max_dist=2.0)
    ...
    grid = info.get_boundary_grid(grid_size=0.04)
