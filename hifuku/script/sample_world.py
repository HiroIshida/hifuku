...
from hifuku.threedim.tabletop import TabletopIKData, TabletopIKProblem
...
class TabletopIKGenerationTask(DataGenerationTask[TabletopIKData]):
    def post_init_hook(self) -> None:
        pass

    def generate_single_data(self) -> TabletopIKData:
        ik_config = IKConfig(disp=False)
        problem = TabletopIKProblem.sample()
        result = problem.solve(av_init, config=ik_config)
        data = TabletopIKData(problem, result)
        return data
